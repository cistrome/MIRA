...
def _plot_rp_model_tails(ax, start_pos, left_decay, right_decay, color = 'lightgrey', linecolor = 'black',
        linewidth = 0.5, alpha = 0.25, bin_size = 50):

    left_extent = -int(10*left_decay)
    left_x = np.arange(-left_extent, 0, left_extent//bin_size)
    left_y = 0.5**(np.abs(left_x) / left_decay)

    right_extend = int(10*right_decay)
    right_x = np.arange(0, right_extent, right_extent//bin_size)
    right_y = 0.5**(np.abs(right_x) / right_decay)

    x, y = np.concatentate([left_x, right_x]), np.concatenate([left_y, right_y])
